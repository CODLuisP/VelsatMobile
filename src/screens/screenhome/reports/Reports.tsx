...
const Reports: React.FC = () => {
    ...
    // Configurar locale al cargar el componente
    useEffect(() => {
        if (Platform.OS === 'ios') {
            // Intentar configurar el locale para iOS
            try {
        ...
        // Inicializar filteredUnits con todas las unidades
        setFilteredUnits(testUnits);
    }, []);

    // Efecto para filtrar unidades basado en el texto de búsqueda
    useEffect(() => {
        console.log('Filtrando unidades con texto:', unitSearchText); // Para debug
        if (unitSearchText.trim() === '') {
            setFilteredUnits(testUnits);
        } else {
            const filtered = testUnits.filter(unit => 
            ...
            console.log('Unidades filtradas:', filtered.length); // Para debug
            setFilteredUnits(filtered);
        }
    }, [unitSearchText]);

    // Efecto para resetear las opciones específicas cuando cambia el reporte seleccionado
    useEffect(() => {
        // Resetear velocidad cuando no es reporte de velocidad
        if (selectedReport !== 2) { // 2 es el ID del reporte de velocidad
            setSpeedValue('');
        }
        
        // Resetear el switch cuando no es reporte de kilometraje
        if (selectedReport !== 3) { // 3 es el ID del reporte de kilometraje
            setAllUnitsEnabled(false);
            // Resetear la animación del switch
    ...
    const reportTypes: ReportType[] = [
        ...
        { id: 4, name: 'Reporte\nDetalle Recorrido', icon: FileText },
    ];


    const handleShowReport = () => {
    // Navegación condicional basada en el reporte seleccionado
    switch (selectedReport) {
        ...
        case 4:
            ...
            break;
        default:
            console.warn(`No se encontró pantalla para el reporte: ${selectedReport}`);
            break;
    }
};

    // Función para toggle del switch (solo funciona si es reporte de kilometraje)
    const toggleAllUnits = () => {
        if (selectedReport !== 3) return; // Solo permitir si es reporte de kilometraje
    ...
    // Funciones para manejar el modal de unidades
    ...
    const formatDate = (date: Date): string => {
        ...
        try {
            ...
            // Convertir formato dd/mm/yyyy a mm/dd/yyyy para mantener consistencia
            const [day, month, year] = formattedDate.split('/');
            return `${month}/${day}/${year}  ${formattedTime}`;
        } catch (error) {
            // Fallback manual si toLocaleDateString falla
    ...
    // Manejadores para los DateTimePickers
    const onDateTimeChange = (event: any, selectedDate?: Date) => {
        ...
        if (selectedDate) {
            if (Platform.OS === 'ios') {
                // En iOS, solo actualizar el estado temporal mientras se hace scroll
    ...
    const handleStartDatePress = () => {
        ...
        setCurrentPickerMode('date');
        setTempStartDate(startDate); // Inicializar fecha temporal
    ...
    const handleEndDatePress = () => {
        ...
        setCurrentPickerMode('date');
        setTempEndDate(endDate); // Inicializar fecha temporal
    ...
    const handleConfirmPicker = () => {
        // Confirmar los cambios de la fecha temporal
        if (currentPickerType === 'start') {
    ...
    // Función para manejar el cambio de texto en velocidad (solo números y solo si es reporte de velocidad)
    const handleSpeedChange = (text: string) => {
        if (selectedReport !== 2) return; // Solo permitir si es reporte de velocidad
    ...
    const renderReportType = (report: ReportType, index: number) => {
        ...
        return (
            ...
            >
                <View style={[styles.reportIcon, isSelected && styles.reportIconSelectedLarge]}>
        ...
        >
            <View style={styles.unitItemLeft}>
                ...
                <View style={styles.unitInfo}>
                    ...
                    <Text style={styles.unitDetails}>{item.plate} • {item.model}</Text>
                </View>
            </View>
        </TouchableOpacity>
    );
                        ...
                        <FlatList
                            ...
                            ListEmptyComponent={() => (
                                <View style={styles.emptyContainer}>
                                    <Text style={styles.emptyText}>No se encontraron unidades</Text>
                                </View>
                            )}
                        />
                    </View>
    ...
    const renderDateTimePicker = () => {
        ...
        const minimumDate = isStartPicker
            ? new Date(2020, 0, 1)
            : startDateSelected ? startDate : new Date(2020, 0, 1);
    ...
    // Funciones para determinar si las opciones están habilitadas
    const isSpeedOptionEnabled = () => selectedReport === 2; // Reporte de velocidad
    const isAllUnitsOptionEnabled = () => selectedReport === 3; // Reporte de kilometraje
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content}>
                ...
                <TouchableOpacity style={styles.unitInputContainer} onPress={handleOpenUnitModal}>
                ...
                {/* Opciones específicas - Siempre visibles pero habilitadas/deshabilitadas según el reporte */}
                <View style={styles.specificOptionsContainer}>
                    <Text style={styles.sectionTitleSpecific}>Opciones específicas</Text>

                    {/* Primera fila - Velocidad con icono */}
                    <View style={styles.optionRow}>
                        ...
                        <View style={styles.inputContainer}>
                            <TextInput
                                ...
                                placeholderTextColor={isSpeedOptionEnabled() ? "#9CA3AF" : "#ccc"}
